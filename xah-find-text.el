     ...
     (lambda (ξpath)
       ...
       (with-temp-buffer
         ...
         (while (search-forward φsearch-str1 nil "NOERROR")
           ...
           (setq ξp3 (match-beginning 0))
           (setq ξp4 (match-end 0))

           (setq ξp1 (max 1 (- ξp3 xah-find-context-char-count-before )))
           (setq ξp2 (min (point-max) (+ ξp4 xah-find-context-char-count-after )))
